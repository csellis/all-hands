This repository uses some of the Recipes on Supabase's page. Namely the Slack Clone with some custom functionality around Rooms

#### Slack Clone SQL Snippet
...
Then I made some modifications with this snippet. You may have to adjust the order.

```
-- ALTER TYPE public.app_permission ADD VALUE 'Rooms.update';
-- ALTER TYPE public.app_permission ADD VALUE 'role_permissions.select';
-- ALTER TABLE public.users ADD CONSTRAINT FK_user_roles FOREIGN KEY (id) REFERENCES user_roles(user_id);
-- ALTER TABLE public.users ADD CONSTRAINT FK_user_roles FOREIGN KEY (role_id) REFERENCES role

-- insert into public.role_permissions (role, permission)
-- values
--     ('admin', 'Rooms.update');

-- create policy "Allow authorized update access" on public.rooms
--   for update using (authorize('Rooms.update', auth.uid()));

-- insert into public.users (id, username)
-- values
--     ('<your_id>', 'chris');

-- create policy "Allow authorized view roles" on public.role_permissions
--   for select using (authorize('role_permissions.select', auth.uid()));

```

Your mileage may vary, please DM me if you experience any schema or RLS issues.
